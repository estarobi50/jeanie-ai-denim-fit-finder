...
**Retrieval Beats Recall for Grounding a Model's Facts**
Brand/product recommendations originally asked Claude to recall real product names from its training data — the source of the broken-link problem worked around earlier (hardcoded brand homepages instead of guessed deep links). The actual fix wasn't a better prompt; it was removing the need to recall at all. `PRODUCT_CATALOG` is a curated, hand-verified list of real product lines per brand/gender/body-shape; `fetchBrands()` filters it to relevant candidates and gives Claude only that list to choose 6 from, validating every returned brand+productName pair against the catalog before it reaches the UI (tier, price range, and URL always come from the catalog, never the model). Claude still does real work — ranking, tier-balancing, and writing grounded persuasive copy — it just can no longer hallucinate a product that doesn't exist. Verified end-to-end against the live API: 6/6 valid, catalog-matched picks on both a women's and a men's test shape.

**`preserve-3d` on a Continuously-Animating Interactive Element Breaks Click Hit-Testing**
The sign-in card's ambient float animation runs infinitely — it never stops, even after the entrance settles — and the card had `transform-style: preserve-3d` set (added for visual cohesion with the 3D entrance, not because anything nested needed shared 3D space). A user reported the submit button wasn't clickable with a mouse, only via keyboard Enter — a telling split, since keyboard submission bypasses pointer hit-testing entirely while a mouse click doesn't. That's the signature of a hit-testing bug, not a logic bug: some browsers unreliably hit-test interactive descendants of an element that's under continuous 3D transform with `preserve-3d` set. Removing `preserve-3d` (unnecessary here — nothing needed shared 3D space with the card's flat 2D children) fixed it; verified by actually clicking the button in a browser and confirming the form navigated, not just by inspecting computed styles.

**A `position:absolute` Tooltip Positioned "Above" an Anchor Pinned to the Viewport Top Renders Off-Screen**
`Tooltip` defaults to `placement="top"`, positioning itself relative to its own wrapping span via `bottom: calc(100% + 10px)`. That's correct for most of the page, but the nav bar (`position: sticky; top: 0`) sits at the very top of the viewport with zero room above it — the Logo and nav-link tooltips there were rendering mostly above `y=0`, clipped by the browser viewport itself. A user saw this as "a black box appears above and can't see it": the tooltip's near-black background peeking a few pixels into view at the very top edge, its actual text pushed out of the visible area above. Fixed by passing `placement="bottom"` to every tooltip anchored inside the sticky nav (Logo, the four nav links, the desktop "Try free" button) — `Logo` gained a `tooltipPlacement` prop so its other usage (the footer, which has plenty of room above it) keeps the default "top" placement. Verified by dispatching a `mouseover` on each tooltip's wrapper span and checking the rendered tooltip's `getBoundingClientRect().top >= 0`, not just reading the source.
